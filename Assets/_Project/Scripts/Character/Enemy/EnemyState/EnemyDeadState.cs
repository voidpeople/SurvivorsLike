...
namespace SurvivorsLike
{
    public class EnemyDeadState : EnemyStateBase
    {
        ...
        public override void Enter()
        {
            ...
            {
                Debug.LogError($"{nameof(EnemyDeadState)}::Enter=> VfxData does not exist. - DeathVfxId: {_ctrl.EnemyData.DeathVfxId}");
                return;
            }
            ...
            if(particleObj == null)
            {
                Debug.LogError($"{nameof(EnemyDeadState)}::Enter=> ParticleEffect does not exist. - PrefabKey: {vfxData.PrefabKey}");
                return;
            }
